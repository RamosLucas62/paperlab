...
type TerminalData = {
  ...
  jev_observations: { at: string; status: string; model: string; result: Record<string, { choice?: string; confidence?: string }> | null; message: string; cost_usd: string | null; cost_status: string; latency_ms: number }[];
...
export default function MarketTerminal({ csrf, onMessage }: { csrf: string; onMessage: (message: string) => void }) {
  ...
  return <section className="terminal-view" aria-label="Terminal JEV">
    <div className="terminal-safety-banner"><span className="terminal-live-dot" /><div><strong>OBSERVAÇÃO · SOMENTE LEITURA</strong><span>Feed Kuru e altura da Monad. Jev classifica o contexto quando habilitado; ordens e assinatura de transação estão desativadas.</span></div><span className="terminal-mode-chip">SEM EXECUÇÃO</span></div>
    ...
    <div className="terminal-main-grid">
      ...
      <article className="terminal-panel terminal-jev-panel"><div className="terminal-panel-heading"><div><span className="terminal-kicker">OPENROUTER · {data?.configuration.jev_model ?? "JEV"}</span><h3>Observações do Jev</h3></div><span className={data?.configuration.jev_configured ? "terminal-config-ok" : "terminal-config-pending"}>{data?.configuration.jev_configured ? "CONFIGURADO" : "CHAVE PENDENTE"}</span></div>
        <div className="terminal-jev-summary"><strong>{data?.control.jev_enabled ? "Jev observando" : "Aguardando ativação"}</strong><p>{data?.control.jev_enabled ? `Uma classificação a cada ${data?.configuration.jev_interval_seconds ?? 120} segundos, sujeita ao orçamento diário.` : "As chamadas são opcionais e podem ter custo. Ative após configurar OPENROUTER_API_KEY no servidor."}</p></div>
        <div className="terminal-jev-list">{data?.jev_observations.slice(0, 5).map((item, index) => <div className="terminal-jev-row" key={`${item.at}-${index}`}><span className={`terminal-jev-indicator ${item.status}`} /><div><strong>{item.status === "ready" ? "Classificação registrada" : item.status === "budget_blocked" ? "Limite de custo atingido" : "Chamada sem resultado"}</strong><small>{time(item.at)} · {item.latency_ms} ms{item.cost_usd ? ` · US$ ${item.cost_usd}` : item.cost_status === "unknown" ? " · custo não informado pelo provedor" : ""}</small>{item.result && <div className="terminal-jev-choices">{Object.entries(item.result).map(([key, value]) => <span key={key}>{jevLabel(key)} <b>{choiceLabel(value.choice)}</b></span>)}</div>}<p>{item.message}</p></div></div>)}{!data?.jev_observations.length && <div className="terminal-empty-note">Nenhuma classificação ainda. O Jev não roda automaticamente.</div>}</div>
    ...
    <article className="terminal-panel terminal-tape-panel"><div className="terminal-panel-heading"><div><span className="terminal-kicker">EVENTOS DO FEED KURU</span><h3>Negociações recentes</h3></div><span className="terminal-tape-count">{data?.events.length ?? 0} eventos</span></div><div className="terminal-tape-scroll"><table className="terminal-tape"><thead><tr><th>HORÁRIO · SP</th><th>LADO AGRESSOR</th><th>PREÇO</th><th>IDENTIFICADOR</th></tr></thead><tbody>{data?.events.map((item, index) => <tr key={`${item.at}-${index}`}><td>{time(item.at)}</td><td><span className={`terminal-side ${item.side === "BUY" ? "buy" : "sell"}`}>{item.side}</span></td><td>{price(item.price)}</td><td className="terminal-hash">{item.tx_hash ? `${item.tx_hash.slice(0, 9)}…${item.tx_hash.slice(-5)}` : "—"}</td></tr>)}</tbody></table>{!data?.events.length && <div className="terminal-empty-note">{loading ? "Carregando eventos…" : "A fita será preenchida quando o feed Kuru publicar negociações."}</div>}</div></article>

    <div className="terminal-bottom-note"><span>ⓘ</span><p>Os lados BUY/SELL acima descrevem o lado agressor reportado pelo feed, não ordens enviadas pelo PaperLab. Jev faz classificação de contexto, não previsão nem recomendação. Nenhum lucro ou resultado de trading é calculado nesta etapa.</p></div>
  </section>;
}

function jevLabel(key: string) { return ({ relevance: "Relevância", risk: "Risco", sufficiency: "Dados" } as Record<string, string>)[key] ?? key; }
function choiceLabel(choice?: string) { return ({ relevant: "relevante", not_relevant: "não relevante", risk_event: "risco", no_risk_event: "sem risco sinalizado", sufficient: "suficientes", insufficient_or_ambiguous: "insuficientes" } as Record<string, string>)[choice ?? ""] ?? choice ?? "—"; }
